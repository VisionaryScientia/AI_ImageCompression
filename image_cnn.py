...
if __name__ == "__main__":
    ...
    autoencoder = make_model(settings.input_size, settings.loss, settings.learning_rate)
    data_path = "./images"
    train, test = create_dataset(data_path, settings.batch_size, settings.inpaint_size, autoencoder.shape)

    if len(sys.argv) > 1 and sys.argv[0] == 'init':
        train_epoch_count = 1
        checkpoint_path, initial_epoch = autoencoder.init_weights(settings.input_size, settings.inpaint_size)
    else:
    ...
    for image_path in files:
        result_image, original_image = evaluate_image(autoencoder, image_path, settings.inpaint_size, settings.max_dim, autoencoder.shape)
